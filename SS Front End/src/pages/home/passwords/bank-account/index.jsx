...
import { getFileData } from "@/redux/slices/fileSlice";

const BankAccountPasswordsList = () => {
  ...
  useEffect(() => {
    dispatch(getPasswordFormDeatils("bank-password"));
  }, [dispatch]);

  useEffect(() => {
    if (renameModal?.id) {
      dispatch(
        getPasswordFormData({ formType: "bank-password", id: renameModal?.id })
      )
        .unwrap()
        .then((res) => {
          setOldFIle(res?.data?.name || res?.data?.account_nick_name);
        });
    }
  }, [dispatch, renameModal?.id]);
              ...
              {formList?.map((item, index) => (
                <>
                  <div
                    ...
                    onDoubleClick={() =>
                      router.push(
                        `/home/passwords-forms/bank-password/${item?.id}`
                      )
                    }
                  ...
                  >
                    ...
                    <p className="font-normal text-sm mt-1 text-[#00000080]">
                      Created On{" "}
...
export default BankAccountPasswordsList;
